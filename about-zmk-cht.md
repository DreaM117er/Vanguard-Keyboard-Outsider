...
- `Lalapad Gen2` by `kot`——[X Link](https://x.com/kot149_/status/2050509588760551813)。
...
3. 再來寫入 `.overlay` 的核心功能設定，將 `I2C` 的代碼設定在「獨立的區塊」：
...
4. 文章前面有提到「`nRF52840` 全功能腳位都能輸出 `I2C` 通訊」，因此它還有一個備援選項，直接開啓開發板內部的 `I2C` 上拉電阻去做設備的頻道匹配：
